...
import argparse
from pformer.preprocessing.preprocessing import resample_data_or_seg
from batchgenerators.utilities.file_and_folder_operations import maybe_mkdir_p
import pformer
from pformer.run.default_configuration import get_default_configuration
...
if __name__ == "__main__":
    ...
    trainer_class = recursive_find_python_class([join(pformer.__path__[0], "training", "network_training")],
                                                trainerclass,
                                                "pformer.training.network_training")

    if trainer_class is None:
        raise RuntimeError("Could not find trainer class in pformer.training.network_training")
    else:
