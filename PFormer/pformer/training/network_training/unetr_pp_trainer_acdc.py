...
import numpy as np
import torch
from pformer.training.data_augmentation.data_augmentation_moreDA import get_moreDA_augmentation
from pformer.training.loss_functions.deep_supervision import MultipleOutputLoss2
from pformer.utilities.to_torch import maybe_to_torch, to_cuda
from pformer.network_architecture.initialization import InitWeights_He
from pformer.network_architecture.neural_network import SegmentationNetwork
from pformer.training.data_augmentation.default_data_augmentation import default_2D_augmentation_params, \
    get_patch_size, default_3D_augmentation_params
from pformer.training.dataloading.dataset_loading import unpack_dataset
from pformer.training.network_training.Trainer_acdc import Trainer_acdc
from pformer.utilities.nd_softmax import softmax_helper
from sklearn.model_selection import KFold
from torch import nn
from torch.cuda.amp import autocast
from pformer.training.learning_rate.poly_lr import poly_lr
from batchgenerators.utilities.file_and_folder_operations import *

# 92.48
from pformer.network_architecture.acdc.pformer_acdc import MyNet


# lsm+sa
# from pformer.network_architecture.acdc.pformer_acdc_lsm_sa import MyNet

# lsm
# from pformer.network_architecture.acdc.pformer_acdc_lsm import MyNet

# lsm+ca
# from pformer.network_architecture.acdc.pformer_acdc_lsm_ca import MyNet
# from pformer.network_architecture.acdc.pformer_acdc_unetr import Unetr

# from fvcore.nn import FlopCountAnalysis


class pformer_trainer_acdc(Trainer_acdc):
    """
    ...
    def initialize_network(self):
        ...
        self.network = MyNet(ch=[128, 128, 256, 512, 1024], k=[256, 128, 32, 8], num_head=[2, 4, 8, 16],
                             depth=[1, 1, 1, 1], deep_supervision=True)
    ...
    #
    #     data_generator = self.tr_gen
    #     data_dict = next(data_generator)
    #     #         print(list(data_dict.keys()))
    #     print(data_dict['properties'])
    #     # print(data_dict['keys'])
    #     # print(data_dict['orig_shape_data'])
    #     #         break
    #     data = data_dict['data']
    #     target = data_dict['target']
    #
    #     MRI_path = '/media/abc/disk11/unetr_plus_plus-main/DATASET_Acdc/unetr_pp_raw/unetr_pp_raw_data/Task01_ACDC/imagesTr/patient001_frame01.nii.gz'
    #
    #     model_weight_path = '/media/abc/disk11/unetr_plus_plus-main/unetr_pp/evaluation/unetr_pp_acdc_checkpoint/unetr_pp/3d_fullres/Task001_ACDC/unetr_pp_trainer_acdc__unetr_pp_Plansv2.1/fold_0/92.48/model_best.model'
    #
    #     i = np.array(data[0, 0, 7, :, :])
    #     print(np.max(i), np.min(i))
    #     i = (i - np.min(i)) / (np.max(i) - np.min(i))
    #     image = np.float32(i)
    #     i = Image.fromarray(np.uint8(image * 255))
    #     print(np.max(i), np.min(i))
    #     i.save("/media/abc/disk11/image.png")
    #
    #     # image = np.float32(np.array(data[0,0,7,:,:]))
    #
    ...
    #     grad_model = self.network.cuda()
    #
    #     weight = torch.load(model_weight_path, map_location='cuda:0')
    #     weight = weight['state_dict']
    #
    #     for name in weight:
    #         print(name)
    #
    #     grad_model.load_state_dict(weight)
    #     grad_model.eval()
    #     use_cuda = torch.cuda.is_available()
    #
    #     output = grad_model(MRI_tensor)[0]
    ...
    #     car_category = 1
    #
    #     print('norm_mask', normalized_masks.shape)
    #     mask = np.float32(normalized_masks.argmax(axis=1))
    #     print(mask.shape)
    #
    #     car_mask = normalized_masks[0, :, :, :].argmax(axis=0).detach().cpu().numpy()
    #     car_mask_uint8 = (255 * np.uint8(car_mask == car_category)[7, :, :])
    #
    #     #         car_mask_float = torch.FloatTensor(mask==car_category)
    #
    #     #         car_mask_float = (np.float32(car_mask == car_category)[[7],:,:])
    #     #         print(car_mask_float.shape)
    #
    #     #         both_images = np.hstack((image, car_mask_uint8))
    #     # #         both_images = np.concatenate((image, car_mask_uint8),axis=2)
    #     # #         both_images=both_images.transpose(1,2,0)
    #
    #     #         a=Image.fromarray(both_images)
    #     #         a.save("/media/abc/disk11/a.png")
    #
    ...
    #
    #         #         print('mask',car_mask_float.shape)
    #
    #     target_layers = [grad_model.D.d3.layer[1]]
    #     targets = [SemanticSegmentationTarget(mask)]
    #
    #     with GradCAM(model=grad_model,
    #                  target_layers=target_layers,
    #                  use_cuda=torch.cuda.is_available()) as cam:
    #         grayscale_cam = cam(input_tensor=MRI_tensor,
    #                             targets=targets)[0, :]
    #         print(grayscale_cam.shape)
    #         # grayscale_cam = scale_cam_image(grayscale_cam, target_size=(160,160))
    #         # gray = Image.fromarray(grayscale_cam)
    #         # gray = gray.save("/media/abc/disk11/gray.png")
    #         # print(grayscale_cam.shape)
    #
    #         cam_image = show_cam_on_image(image, grayscale_cam, use_rgb=True, image_weight=0.75)
    #         print(cam_image.shape)
    #
    #     c = Image.fromarray(cam_image)
    #
    #     c.save("/media/abc/disk11/c.png")
