...
from pformer.network_architecture.dynunet_block import UnetOutBlock, UnetResBlock
from pformer.network_architecture.synapse.model_components import UnetrPPEncoder, UnetrUpBlock
...
class PatchEmbed(nn.Module):
    def __init__(self, patch_size=2, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        stride1 = [patch_size[0] // 2, patch_size[1], patch_size[2]]
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, 40 * 40 * 14, embed_dim))
...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.dim = dim
        self.res = DWconv(in_ch=dim, out_ch=dim, kernel_size=7, stride=1, padding=3)
...
class final_patch_expanding(nn.Module):
    def __init__(self, dim, num_class, patch_size):
        ...
        self.up = nn.Sequential(nn.Upsample(scale_factor=(1, 1, 1), mode='trilinear'),
                                DWconv(dim, out_ch=32, kernel_size=3, stride=1, padding=1), LN(32), nn.GELU(),
                                nn.Upsample(scale_factor=(1, 2, 2), mode='trilinear'),
                                DWconv(32, 4, kernel_size=1, stride=1)
                                )
...
class Encoder(nn.Module):
    def __init__(self, ch=[64, 128, 256, 512, 1024], k=[2048, 512], num_head=[4, 8, 16, 32], depth=[2, 2, 1, 1]):
        ...
        self.d1 = Downsampling(ch[1], conv=True, kernel_size=[1, 3, 3], stride=[1, 2, 2], padding=[0, 1, 1])
        self.e2 = encoder_layer(in_ch=ch[1], out_ch=ch[2], k=k[1], num_head=num_head[1], depth=depth[1])
        self.d2 = Downsampling(ch[2], conv=True, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 1])
        self.e3 = encoder_layer(in_ch=ch[2], out_ch=ch[3], k=k[2], num_head=num_head[2], depth=depth[2])
        self.d3 = Downsampling(ch[3], conv=True, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 1])
...
class Decoder(nn.Module):
    def __init__(self, ch=[64, 128, 256, 512, 1024], k=[2048, 512], num_head=[4, 8, 16, 32], depth=[1, 1, 1, 2]):
        ...
        self.u3 = Upsampling(ch=ch[4], conv=True, kernel_size=[2, 2, 2], stride=[2, 2, 2], padding=[0, 0, 0])
        self.d3 = encoder_layer(in_ch=ch[3] * 2, out_ch=ch[3], num_head=num_head[2], depth=depth[2], k=k[2])
        self.u2 = Upsampling(ch=ch[3], conv=True, kernel_size=[2, 2, 2], stride=[2, 2, 2], padding=0)
        self.d2 = encoder_layer(in_ch=ch[2] * 2, out_ch=ch[2], num_head=num_head[1], depth=depth[1], k=k[1])
        self.u1 = Upsampling(ch=ch[2], conv=True, kernel_size=[1, 2, 2], stride=[1, 2, 2], padding=0)
        ...
        self.seg = final_patch_expanding2(dim=ch[1], num_class=4, patch_size=(2, 4, 4))
        self.seg1 = final_patch_expanding(dim=ch[2], num_class=4, patch_size=(2, 4, 4))
        self.seg2 = final_patch_expanding(dim=ch[3], num_class=4, patch_size=(2, 4, 4))
...
if __name__ == "__main__":
    network = MyNet(ch=[128, 128, 256, 512, 1024], k=[512, 128, 64, 32], num_head=[2, 4, 8, 16], depth=[1, 1, 1, 1],
                    deep_supervision=True)
    macs, params = get_model_complexity_info(network, (1, 16, 160, 160), as_strings=True, print_per_layer_stat=True)
